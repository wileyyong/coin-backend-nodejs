...
const Controller = {
  ...
  async getOneUser(req, res) {
    try {
      ...
      where = {
        [Op.or]: [{ wallet: user_id }, { link: user_id }, { _id: user_id }]
      };
      ...
      if (req.user) {
        if (req.user.id == user_id) {
          ...
          user.followed = current_user.following
            ? current_user.following.includes(user_id)
            : null;
        }
      } else {
        user.followed = false;
  ...
  async getUserItems(req, res) {
    try {
      ...
      var user = await Users.findOne({
        where: {
          [Op.or]: [{ wallet: user_id }, { _id: user_id }]
        },
  ...
  async updateUserSettings(req, res) {
    try {
      ...
      if (req.files && req.files.avatar) {
        set.avatar = await helpers.uploadFile(
          req.files.avatar,
          req.user.id,
          "content/avatar"
        );
  ...
  async changeCoverImage(req, res) {
    try {
      ...
      if (req.files && req.files.image) {
        var cover = await helpers.uploadFile(
          req.files.image,
          req.user.id,
          "content/cover"
        );
  ...
  async getTopUsers(req, res) {
    try {
      ...
      if (users.length > 50) users = users.splice(0, 50);
  ...
  async pumlxApproved(req, res) {
    try {
      ...
      if (!update || update === [0])
        return res.status(422).send({ error: "Approved error" });
